...
const getFilterOptions = async (req, res) => {
  try {
    ...
    // Lấy tên danh mục
    const [[cat]] = await db.query("SELECT name FROM categories WHERE id = ?", [
      categoryId,
    ]);
    ...
    if (allowedSpecs.length === 0) {
      ...
      return res.json({
        success: true,
        data: { brands: brands.map((b) => b.brand), specs: {} },
      });
    }
    ...
    Object.keys(filters).forEach((k) => {
      if (filters[k].length === 0) delete filters[k];
    });
...
const setFlashSale = async (req, res) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0)
      return res
        .status(400)
        .json({ success: false, message: "Thiếu danh sách sản phẩm" });
    if (!durationHours || durationHours < 1)
      return res
        .status(400)
        .json({ success: false, message: "Thời gian ít nhất 1 giờ" });
