...
exports.getCart = async (req, res) => {
  try {
    ...
    let [carts] = await db.query("SELECT id FROM cart WHERE user_id = ?", [
      req.user.userId,
    ]);
  ...
  try {
    ...
    if (!Number.isInteger(quantity) || quantity < 1) {
      return res.status(400).json({
        success: false,
        message: "Số lượng thêm vào giỏ phải là số nguyên lớn hơn 0",
      });
    }
    ...
    if (!req.user.userId) {
      ...
      return res
        .status(401)
        .json({ success: false, message: "Token không có userId" });
    }
    ...
    if (!carts || carts.length === 0) {
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          message: "Lỗi tạo giỏ hàng: " + dbError.message,
        });
      }
    } else {
      cartId = carts[0].id;
    ...
    if (existing && existing.length > 0) {
      ...
      // ✅ Check tổng số lượng trong giỏ không vượt tồn kho
      if (newTotalQty > product.stock) {
        return res.status(400).json({
          success: false,
          message: `Chỉ còn ${product.stock} sản phẩm trong kho, bạn đã có ${currentQty} sản phẩm trong giỏ`,
      ...
      // ✅ Nếu là sản phẩm mới trong giỏ thì vẫn phải check stock
