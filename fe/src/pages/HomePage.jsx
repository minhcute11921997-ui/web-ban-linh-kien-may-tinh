...
const SLIDES = [
  { id: 1, title: "🔥 Siêu sale linh kiện PC",    subtitle: "Giảm đến 30% cho CPU & VGA",       bg: "from-blue-600 to-blue-800",    accent: "💻" },
  { id: 2, title: "⚡ RAM DDR5 giá sốc",           subtitle: "Nâng cấp hiệu năng vượt trội",     bg: "from-purple-600 to-indigo-800", accent: "🚀" },
  { id: 3, title: "🎮 VGA RTX 40 Series",          subtitle: "Trải nghiệm gaming đỉnh cao",      bg: "from-green-600 to-teal-800",   accent: "🎯" },
  { id: 4, title: "💾 SSD NVMe Gen4/Gen5",         subtitle: "Tốc độ đọc ghi siêu nhanh",       bg: "from-orange-500 to-red-700",   accent: "⚡" },
  { id: 5, title: "Miễn phí vận chuyển",           subtitle: "Cho đơn hàng từ 500K toàn quốc",  bg: "from-pink-500 to-rose-700",    accent: "🚚" },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    setActiveCategory("all"); setActiveBrand("all");
    setPriceMin(MIN_PRICE);   setPriceMax(MAX_PRICE);
    setSortOption("id-DESC"); setCurrentPage(1);
    setActiveSubFilters({});  setSearchInput(""); setSearchQuery("");
  }, [location.pathname]);
  ...
  useEffect(() => {
    ...
    slideInterval.current = setInterval(() => setSlideIndex((p) => (p + 1) % SLIDES.length), 4000);
    return () => clearInterval(slideInterval.current);
  }, [slidePaused]);

  // Danh mục
  useEffect(() => {
    getAllCategories().then((res) => { if (res.data.success) setCategories(res.data.data); }).catch(() => {});
  }, []);

  // Debounce giá
  useEffect(() => {
    const t = setTimeout(() => { setDebouncedMin(priceMin); setDebouncedMax(priceMax); }, 400);
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/api/products/featured`)
      .then((r) => r.json()).then((d) => { if (d.success) setFeaturedProducts(d.data); }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      setFlashSaleTimeLeft({ h: Math.floor(rem / 3600000), m: Math.floor((rem % 3600000) / 60000), s: Math.floor((rem % 60000) / 1000) });
      if (rem === 0) { setFlashSaleProducts([]); clearInterval(t); }
    }, 1000);
    ...
    if (key === "all") {
      fetch(`${import.meta.env.VITE_API_URL}/api/products?sortBy=id&sortOrder=DESC`)
        .then((r) => r.json()).then((d) => {
          if (d.success) {
            const brands = [...new Set(d.data.map((p) => p.brand).filter(Boolean))].sort();
      ...
      fetch(`${import.meta.env.VITE_API_URL}/api/products/filters/${catId}`)
        .then((r) => r.json()).then((d) => {
          if (d.success) {
            setCategoryBrandsMap((prev) => ({ ...prev, [catId]: d.data.brands }));
            setCategoryFiltersMap((prev) => ({ ...prev, [catId]: d.data.specs }));
          }
        });
    }
  };

  const handleCategoryMouseEnter = (catId) => { if (hoverTimeout) clearTimeout(hoverTimeout); setHoveredCategory(catId); fetchCategoryData(catId); };
  const handleCategoryMouseLeave = () => { const t = setTimeout(() => setHoveredCategory(null), 200); setHoverTimeout(t); };
  const handleDropdownMouseEnter = () => { if (hoverTimeout) clearTimeout(hoverTimeout); };
  const handleDropdownMouseLeave = () => setHoveredCategory(null);

  const handleSelectBrand    = (catId, brand) => { setActiveCategory(catId); setActiveBrand(brand); setActiveSubFilters({}); setHoveredCategory(null); };
  const handleSelectCategory = (catId)        => { setActiveCategory(catId); setActiveBrand("all"); setActiveSubFilters({}); setHoveredCategory(null); };
  const toggleSubFilter = (catId, key, value) => {
    setActiveCategory(catId);
    setActiveSubFilters((prev) => { if (prev[key] === value) { const n = { ...prev }; delete n[key]; return n; } return { ...prev, [key]: value }; });
    setCurrentPage(1); setHoveredCategory(null);
  };

  useEffect(() => { setCurrentPage(1); }, [activeCategory, activeBrand, debouncedMin, debouncedMax, sortOption, activeSubFilters, searchQuery]);
  ...
  useEffect(() => {
    ...
    if (Object.keys(activeSubFilters).length > 0) p.append("specs", JSON.stringify(activeSubFilters));
    const [sortBy, sortOrder] = sortOption.split("-");
    p.append("sortBy", sortBy); p.append("sortOrder", sortOrder);
    p.append("limit", ITEMS_PER_PAGE); p.append("page", currentPage);

    fetch(`${import.meta.env.VITE_API_URL}/api/products?${p.toString()}`)
      .then((r) => r.json()).then((d) => { if (d.success) { setProducts(d.data); setTotalPages(d.totalPages || 1); setTotalProducts(d.total || 0); } setLoading(false); })
    ...
    fetch(`${import.meta.env.VITE_API_URL}/api/products/on-sale`)
      .then((r) => r.json()).then((d) => {
        if (d.success && d.data.length > 0) {
          ...
          if (d.data[0].discount_expires_at) flashSaleEndTime.current = new Date(d.data[0].discount_expires_at).getTime();
        }
      }).catch(() => {});
  }, []);

  const handleAddToCart = async (product) => {
    if (!token) { toast.warning("Vui lòng đăng nhập để thêm giỏ hàng!"); navigate("/login"); return; }
    try { await addItem(product.id, 1); toast.success(`Đã thêm "${product.name}" vào giỏ hàng!`); }
    catch (err) { toast.error(err?.message || "Thêm giỏ hàng thất bại!"); }
  };
    ...
    <div className="p-4">
      ...
      >
        <div className="flex transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${slideIndex * 100}%)` }}>
          {SLIDES.map((slide) => (
            <div key={slide.id} className={`w-full flex-shrink-0 bg-gradient-to-r ${slide.bg} px-8 py-10 md:py-14 flex items-center justify-between`}>
              <div>
      ...
      {featuredProducts.length > 0 && (
        <div className="mb-8">
          ...
          <div className="relative group">
            ...
            <div ref={featuredRef} className="flex flex-row gap-3 overflow-x-auto scroll-smooth pb-2"
        ...
        <div className="mb-8">
          ...
          <div className="relative group">
            ...
            <div ref={flashSaleRef} className="flex flex-row gap-3 overflow-x-auto scroll-smooth pb-2"
              style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
              {flashSaleProducts.map((p) => <FlashSaleCard key={p.id} product={p} />)}
            </div>
            <ArrowBtn direction="right"
              onClick={() => flashSaleRef.current?.scrollBy({ left: 300, behavior: "smooth" })}
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover:opacity-100" />
      ...
      <div className="flex gap-2 flex-wrap mb-4 items-center justify-between bg-white py-3 -mx-4 px-4 shadow-sm">
        <div className="flex gap-2 flex-wrap items-center">
          <button onClick={() => handleSelectCategory("all")}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${activeCategory === "all" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
            Tất cả
          </button>
          ...
          {categories.map((cat) => (
            <div key={cat.id} className="relative"
              ...
              <button onClick={() => handleSelectCategory(cat.id)}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${activeCategory === cat.id ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                {cat.name} ▾
              </button>
              ...
              {hoveredCategory === cat.id && (
                <div onMouseEnter={handleDropdownMouseEnter} onMouseLeave={handleDropdownMouseLeave}
                  ...
                  <button onClick={() => handleSelectBrand(cat.id, "all")}
                    className={`w-full text-left px-4 py-2 text-sm transition-colors ${activeCategory === cat.id && activeBrand === "all" ? "bg-blue-50 text-blue-600 font-medium" : "text-gray-700 hover:bg-blue-50 hover:text-blue-600"}`}>
                    Tất cả {cat.name}
                  </button>
                  {(categoryBrandsMap[cat.id] || []).map((brand) => (
                    <button key={brand} onClick={() => handleSelectBrand(cat.id, brand)}
                      className={`w-full text-left px-4 py-2 text-sm transition-colors ${activeCategory === cat.id && activeBrand === brand ? "bg-blue-50 text-blue-600 font-medium" : "text-gray-700 hover:bg-blue-50 hover:text-blue-600"}`}>
                      {brand}
                    </button>
                  ))}
                  ...
                  {(() => {
                    ...
                    return Object.entries(specs).map(([specName, values]) => (
                      <div key={specName}>
                        ...
                        {values.map((v) => (
                          <button key={v.value} onClick={() => toggleSubFilter(cat.id, specName, v.value)}
                            className={`w-full text-left px-4 py-2 text-sm transition-colors ${activeCategory === cat.id && activeSubFilters[specName] === v.value ? "bg-blue-50 text-blue-600 font-medium" : "text-gray-700 hover:bg-blue-50 hover:text-blue-600"}`}>
        ...
        <div className="flex gap-2 items-center flex-1 max-w-sm">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Tìm kiếm..." value={searchInput}
          ...
          <button onClick={() => { setSearchQuery(searchInput); setCurrentPage(1); }}
        ...
        <div className="flex-1 min-w-[280px]">
          <div className="flex items-center justify-between mb-2">
            <span className="flex items-center gap-1.5 text-sm font-medium text-gray-500">
              <SlidersHorizontal size={14} className="text-blue-500" /> Khoảng giá:
            </span>
          ...
          <div className="relative h-6 flex items-center"
            onClick={(e) => {
              ...
              const val  = Math.round((pct * (MAX_PRICE - MIN_PRICE) + MIN_PRICE) / PRICE_STEP) * PRICE_STEP;
              const cval = Math.max(MIN_PRICE, Math.min(MAX_PRICE, val));
              if (Math.abs(cval - priceMin) < Math.abs(cval - priceMax))
                setPriceMin(Math.max(MIN_PRICE, Math.min(cval, priceMax - PRICE_STEP)));
              else
                setPriceMax(Math.min(MAX_PRICE, Math.max(cval, priceMin + PRICE_STEP)));
            }}>
            ...
            <div className="absolute h-1.5 bg-blue-500 rounded-full"
              style={{ left: `${((priceMin - MIN_PRICE) / (MAX_PRICE - MIN_PRICE)) * 100}%`, right: `${100 - ((priceMax - MIN_PRICE) / (MAX_PRICE - MIN_PRICE)) * 100}%` }} />
            <input type="range" min={MIN_PRICE} max={MAX_PRICE} step={PRICE_STEP} value={priceMin}
            ...
            <input type="range" min={MIN_PRICE} max={MAX_PRICE} step={PRICE_STEP} value={priceMax}
          ...
          <div className="flex gap-1.5 mt-2">
            {[
              ...
              <button key={i} onClick={() => { setPriceMin(r.min); setPriceMax(r.max); }}
                className={`px-2 py-1 rounded text-xs transition-all ${priceMin === r.min && priceMax === r.max ? "bg-blue-600 text-white" : "bg-white text-gray-500 hover:bg-blue-50 border border-gray-200"}`}>
                {r.label}
              </button>
            ))}
          </div>
            ...
            <div className="flex justify-center items-center gap-2 mt-8">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button key={page} onClick={() => setCurrentPage(page)}
                  className={`w-9 h-9 rounded-lg text-sm font-medium transition-all ${currentPage === page ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                  {page}
                </button>
              ))}
