...
export default function AdminDiscounts() {
  ...
  // bỏ chặn số 0 khi đang gõ dở, chỉ chặn > 100
  const handleValueChange = (e) => {
    const raw = e.target.value;
    if (raw !== "" && !/^\d*\.?\d*$/.test(raw)) return;
    if (form.type === "percent" && raw !== "") {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Tổng: {discounts.length} mã &nbsp;|&nbsp; Đang hoạt động:{" "}
            {
              discounts.filter(
                (d) =>
                  d.active &&
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {/* input value với chặn 1-100 khi percent */}
                <div>
                  <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide block mb-1">
                    Giá trị{" "}
                    {form.type === "percent" && (
                  ...
                  <div className="relative">
                    <input
                      ...
                      onBlur={() => {
                        if (form.type === "percent" && form.value !== "") {
                          const num = parseFloat(form.value);
                          if (!isNaN(num)) {
                            if (num > 100) setForm({ ...form, value: "100" });
                            if (num < 1) setForm({ ...form, value: "1" });
                          }
                        }
                      }}
                      placeholder={
                        form.type === "percent" ? "1 – 100" : "50000"
                      }
                      className={`w-full border rounded-xl px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 ${
                        form.type === "percent" &&
                        form.value !== "" &&
                        (parseFloat(form.value) < 1 ||
                ...
                <div className="bg-blue-50 rounded-xl px-4 py-3 text-sm text-blue-700 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    {form.min_order_value > 0 &&
                      ` cho đơn từ ${fmt(form.min_order_value)}₫`}
                    {form.expires_at &&
                      ` — HSD: ${new Date(form.expires_at).toLocaleDateString(
                        "vi-VN"
                      )}`}
                  </div>
                </div>
