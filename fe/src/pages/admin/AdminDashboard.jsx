  ...
  if (active && payload && payload.length)
    return (
      <div className="bg-white border border-blue-100 rounded-xl shadow-lg px-4 py-3 text-sm">
        ...
        <p className="font-bold text-blue-600">{Number(payload[0].value).toLocaleString("vi-VN")}₫</p>
      </div>
    );
  return null;
};
...
const AdminDashboard = () => {
  ...
  const cards = [
    { label: "Sản phẩm",   value: stats?.totalProducts || 0,
      color: "from-blue-500 to-blue-600",   link: "/admin/products" },
    { label: "Đơn hàng",   value: stats?.totalOrders || 0,
      color: "from-blue-600 to-blue-700",   link: "/admin/orders",
      badge: stats?.pendingOrders > 0 ? `${stats.pendingOrders} chờ xử lý` : null },
    { label: "Doanh thu",  value: `${Number(stats?.totalRevenue || 0).toLocaleString("vi-VN")}₫`,
      color: "from-blue-700 to-indigo-700", link: "/admin/revenue" },
    { label: "Người dùng", value: stats?.totalUsers || 0,
      color: "from-indigo-500 to-indigo-600", link: "/admin/users" },
    { label: "Hết hàng",  value: stats?.outOfStock || 0,
      color: "from-red-500 to-red-600",     link: "/admin/products" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-6">
        {cards.map((card, i) => (
          <div key={i} onClick={() => card.link && navigate(card.link)}
            ...
            {card.badge && (
              <span className="bg-white/20 text-xs px-2 py-0.5 rounded-full block w-fit mb-2">{card.badge}</span>
            )}
