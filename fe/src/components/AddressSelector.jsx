...
const AddressSelector = ({ onChange, defaultValue = null }) => {
  ...
  // Tránh gọi onChange trên lần render đầu tiên (chỉ gọi khi user thay đổi)
  const isMounted = useRef(false);

  // ── Load tỉnh 1 lần ──
  useEffect(() => {
    fetch(`${BASE}/?depth=1`).then(r => r.json()).then(setProvinces);
  }, []);
  ...
  const handleProvince = (e) => {
    ...
    setProvCode(code); setProvName(name);
    setDistCode('');   setDistName('');   // reset huyện + xã
  ...
  const handleDistrict = (e) => {
    ...
    setDistCode(code); setDistName(name);
    setWardCode('');   setWardName('');   // reset xã
  };

  const handleWard = (e) => {
    const code = e.target.value;
    const name = wards.find(w => String(w.code) === code)?.name ?? '';
    setWardCode(code); setWardName(name);
  };

  const cls = "w-full px-4 py-2.5 border border-gray-300 rounded-lg " +
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <select value={distCode} onChange={handleDistrict} disabled={!districts.length} className={cls}>
      ...
      <div>
        ...
        <select value={wardCode} onChange={handleWard} disabled={!wards.length} className={cls}>
